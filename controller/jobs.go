...
func (c *controllerAPI) RunJob(ctx context.Context, w http.ResponseWriter, req *http.Request) {
	...
	attach := strings.Contains(req.Header.Get("Upgrade"), "drycc-attach/0")
	...
	env["DRYCC_APP_ID"] = app.ID
	env["DRYCC_RELEASE_ID"] = release.ID
	env["DRYCC_PROCESS_TYPE"] = ""
	env["DRYCC_JOB_ID"] = id
	...
	metadata["drycc-controller.app"] = app.ID
	metadata["drycc-controller.app_name"] = app.Name
	metadata["drycc-controller.release"] = release.ID
	job := &host.Job{
		ID:       id,
		Metadata: metadata,
