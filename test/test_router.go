...
func (s *RouterSuite) TestAdditionalHttpPorts(t *c.C) {
	...
	t.Assert(x.drycc("/", "-a", "router", "env", "set", "ADDITIONAL_HTTP_PORTS=8080", "ADDITIONAL_HTTPS_PORTS=8081"), Succeeds)
	...
	t.Assert(x.drycc("/", "-a", "dashboard", "route", "add", "http", "-s", "dashboard-web", "-p", "8080", "dashboard."+x.Domain), Succeeds)
	...
	t.Assert(err, c.IsNil)
	certRoute := x.drycc("/", "-a", "dashboard", "route", "add", "http", "-s", "dashboard-web", "-p", "8081", "--tls-cert", certPath, "--tls-key", keyPath, "dashboard."+x.Domain)
...
func (s *RouterSuite) TestCustom503ErrorPage(t *c.C) {
	...
	watcher, err := x.controller.WatchJobEvents("router", "")
	t.Assert(err, c.IsNil)
	t.Assert(x.drycc("/", "-a", "router", "env", "set", "ERROR_503_PAGE_URL=https://www.google.com"), Succeeds)
	t.Assert(watcher.WaitFor(ct.JobEvents{"app": {ct.JobStateUp: 1, ct.JobStateDown: 1}}, 10*time.Second, nil), c.IsNil)

	// make sure the route returns 503
	t.Assert(x.drycc("/", "-a", "dashboard", "scale", "web=0"), Succeeds)
