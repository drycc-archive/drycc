...
const (
	// DefaultServiceName is the service name used if DRYCC_REDIS is empty.
	...
	release := &ct.Release{
		...
		Processes: map[string]ct.ProcessType{
			"redis": {
				...
				Args:    []string{"/bin/start-drycc-redis", "redis"},
				Service: serviceName,
			},
		},
		Env: map[string]string{
			"DRYCC_REDIS":    serviceName,
...
func (h *Handler) serveDeleteCluster(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	...
	appName := release.Env["DRYCC_REDIS"]
	if appName == "" {
