...
import (
	...
	"github.com/drycc/libnetwork/ipallocator"
	"github.com/drycc/libnetwork/netutils"
...
func InitLibcontainerBackend() (error){
        runtime.GOMAXPROCS(1)
        runtime.LockOSThread()
        factory, _ := libcontainer.New("")
        return factory.StartInitialization()
}
...
func (l *LibcontainerBackend) AppendCapabilities(linux_capabilities...string) {
	l.capabilities.Bounding = append(l.capabilities.Bounding, linux_capabilities...)
	l.capabilities.Inheritable = append(l.capabilities.Inheritable, linux_capabilities...)
	l.capabilities.Effective = append(l.capabilities.Effective, linux_capabilities...)
	l.capabilities.Permitted = append(l.capabilities.Permitted, linux_capabilities...)
	l.capabilities.Ambient = append(l.capabilities.Ambient, linux_capabilities...)
}


func (l *LibcontainerBackend) ConvertDevices(devices []*host.Device) (*[]*configs.Device) {
        length := len(devices)
        result := make([]*configs.Device, length)
        for i := 0; i < length; i++ {
                device := *devices[i]
                result[i] = &configs.Device{
                        Type: device.Type,
                        Path: device.Path,
                        Major: device.Major,
                        Minor: device.Minor,
...
func (l *LibcontainerBackend) Run(job *host.Job, runConfig *RunConfig, rateLimitBucket *RateLimitBucket) (err error) {
	...
	l.AppendCapabilities(*job.Config.LinuxCapabilities...)
	...
	config := &configs.Config{
		...
		Cgroups: &configs.Cgroup{
			...
			Resources: &configs.Resources{
				AllowedDevices: *l.ConvertDevices(*job.Config.AllowedDevices),
		...
		Devices: *l.ConvertDevices(*job.Config.AutoCreatedDevices),
	...
	if job.Config.HostNetwork {
		...
		l.AppendCapabilities("CAP_NET_ADMIN")
	} else {
	...
	if spec, ok := job.Resources[resource.TypeCPU]; ok && spec.Limit != nil {
		config.Cgroups.Resources.CpuShares = uint64(milliCPUToShares(*spec.Limit))
	}
...
func setupCGroups(partitions map[string]int64) error {
	...
	for _, subsystem := range subsystems {
		if _, err := cgroups.FindCgroupMountpoint(cgroupRoot, subsystem); err == nil {
