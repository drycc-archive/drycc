#!/usr/bin/env bash
#
# A script to clone a Drycc app.
#
# Typical usage would be:
#
#   drycc-clone.sh my-app my-app-staging
#
...
Clone a Drycc app from SRC_APP to DST_APP.
...
main() {
  ...
  drycc -c "${src_cluster}" -a "${src_app}" release show --json > "${tmp}/src.json"
  ...
  drycc -c "${dst_cluster}" -a "${dst_app}" create --remote "" "${dst_app}"

  info "creating destination release"
  # set an env var to create an inital release which can then be updated
  drycc -c "${dst_cluster}" -a "${dst_app}" env set "CLONE=true"
  drycc -c "${dst_cluster}" -a "${dst_app}" release update "${tmp}/dst.json"
...
necessary with 'drycc -c ${dst_cluster} -a ${dst_app} resource add ...'.

Run 'drycc -c ${dst_cluster} -a ${dst_app} remote add <remote>' to add a git remote for
